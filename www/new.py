...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        #print('1st attrs:', attrs)
        #把数据库类别从属性中抛掉
        for k in mappings.keys():
            attrs.pop(k)
        #print('2nd attrs:', attrs)
        escaped_fileds = list(map(lambda f: '`%s`' % f, fields))
        #print('escaped_fileds:', escaped_fileds)
...
class User(Model):
    ...
    image = StringField(ddl='varchar(500)', default='about:blank')
...
#创建实例
async def test():
    await create_pool(loop=loop,host='localhost', port=3306, user='root', password='password', db='awesome')
    user = User(id=10, name='Ablin', email='Ablin@example.com')
    await user.save()
    ...
    print(2, r)
    r = await User.findAll(name='Ablin', id='10')
    print(3, r)
